...
<c:forEach var="item" items="<%=SystemManage.getInstance().getArticleCategory()%>">
    <li <c:if test="${e.catename eq item.catename}">class="active"</c:if>><a href="<%=path%>/laws/${item.code}">${item.catename}</a></li>
    </c:forEach>
            </ul>
        </div>
    </div>
    <div class="warp_right">
        <div class="breadcrumb">
            ...
            >
            <a href="<%=path%>/laws?categoryId=${e.categoryId}">${e.catename}</a>
        </div>
        <div style="overflow: hidden;">
            <div class="article_content">
                ...
                <div class="previous">上一篇：<c:choose><c:when test="${previous.id != 0}"><a href="<%=path%>/laws/${previous.id}">${previous.title}</a></c:when><c:otherwise>没有了</c:otherwise></c:choose></div>
                <div class="next">下一篇：<c:choose><c:when test="${next.id != 0}"><a href="<%=path%>/laws/${next.id}">${next.title}</a></c:when><c:otherwise>没有了</c:otherwise></c:choose></div>
            </div>
        </div>
    </div>
</div>
